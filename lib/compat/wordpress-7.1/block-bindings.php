...
if ( ! function_exists( 'gutenberg_cover_bindings_is_active' ) ) {
	 ...
	 * `useCoverBindingState`. A binding is "active" when BOTH the `id` and
	 * `url` attributes are bound to the same source instance — same `source`
	 * string AND `==`-equal `args`. Loose equality (`==`) on `args` matches
	 * the JS-side `JSON.stringify` parity check and is the correct semantic
	 * for the small associative bags that source `args` contain in practice;
	 * element ordering does not affect equality.
	 *
	 * A `__default` entry sourced from `core/pattern-overrides` is its own
	 * active-binding indicator: Core's `process_block_bindings` already
	 * materialises it into per-attribute slots before resolution, so the
	 * Cover render filter sees resolved `id`/`url` values without this helper
	 * having to expand `__default` itself.
	 *
	...
	function gutenberg_cover_bindings_is_active( array $attrs ): bool {
		...
		// A `__default` pattern-overrides binding implies both `id` and `url`
		// are bound to the same source instance once Core's
		// `process_block_bindings` expands it.
